...
(function () {
  ...
  function renderResults(repoUrl, data) {
    ...
    // Wire up copy-as-markdown button (copies to clipboard)
    const copyMdBtn = document.getElementById('copy-md-btn');
    if (copyMdBtn) {
      const newCopyMdBtn = copyMdBtn.cloneNode(true);
      copyMdBtn.parentNode.replaceChild(newCopyMdBtn, copyMdBtn);
      newCopyMdBtn.addEventListener('click', function () {
        const o = (data.meta && data.meta.owner) || 'unknown';
        const r = (data.meta && data.meta.repo)  || 'repo';
        const v = (data.verified     || []).length;
        const u = (data.unverifiable || []).length;
        ...
        lines.push('');
        if (data.notes && data.notes.read) { lines.push('> ' + data.notes.read); lines.push(''); }
        ...
        for (var bi = 0; bi < bkts.length; bi++) {
          ...
          lines.push('## ' + bkt.label + ' (' + items.length + ')'); lines.push('');
        ...
        navigator.clipboard.writeText(md).then(function () {
          setText(newCopyMdBtn, 'copied! \u2713');
          setTimeout(function () { setText(newCopyMdBtn, 'copy as .md \u2197'); }, 2000);
        }).catch(function () {
          setText(newCopyMdBtn, 'failed \u2715');
          setTimeout(function () { setText(newCopyMdBtn, 'copy as .md \u2197'); }, 2000);
        });
      });
    }
